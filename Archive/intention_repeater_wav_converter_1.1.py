...
sys.stdout.write('Intention Repeater WAV Converter v1.1 software created by Thomas Sweet.\n')
...
if duration_param != 'Until Stopped':
    duration_sec = duration_param[7:8]
    duration_minutes = duration_param[4:5]
    duration_hours = duration_param[1:2]
        ...
        while sample_num != total_samples:
            for element in string_to_write_value:
                ...
                if total_samples == sample_num:
                    ...
                    sys.stdout.write('Num Samples Written: ' + human_format(total_samples) + '\n')
                    obj.close()
                    quit()
            num_writes += 1
